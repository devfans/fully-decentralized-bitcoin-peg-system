...
* When an operator made invalid BTC transfers, or failed to process the peg out requests in time. The staked layer 2 token(the collateral) will be distributed into users proportionally who requested to peg out with this operator, and the rest of collateral will be processed through liquidation.
