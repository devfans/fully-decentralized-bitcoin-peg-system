...
Both my proposal and the sBTC design involve the lock of layer 2 token, however, there're some differences and here's to describe the tradeoffs.

* There's a group of signer to manage the peg wallet in sBTC, while in my proposal, peg operators hold their own ones respectively. This difference leads to different properties on each other. sBTC signers need to reach on a consensus on shift of group of signers to transfer fund to a new one. This indicates the membership change needs the permission of previous group.   It's occationally somehow hard for a group of signers to reach such a consensus due to conflicts of interests or lack of enough liveness of signers. In addition the signer group needs to meet such a threshold level too even the peg-out request is verified by the protocol. What it gains is clear too. To compromise such a peg wallet will be hard when not incentivized. The single peg wallet in sBTC can also meet the need of peg-out request when the volume is high.  While in my proposal, there's no such a group controlled wallet. Any operator can join or leave the system freely without the requirement of any permission. The tradeoff is that it's easy for a single operator to malicious behave as long as bearing the loss of the collateral and forfeiting the reward. For high volume peg-out requests, a set of operators will be chosen to fulfill the requirement.
* sBTC has designed a degraded script path to tolerate the offline signers and other situations when the target threshold level can not be met. The tradeoff is the degrade of security to bring further liveness, while in my proposal, the liveness is ensured by the collateral which has the more value than the BTC hold in peg wallets and the reward incentive from the consensus or peg users. The reward incentive applies to sBTC too.
* There's no clear treatment specified on the collateral in sBTC's design. While in my proposal, there's clear defined liquidation mechanism to slash misbehaved operators. At the worst case, if no slash on signers in sBTC, the signer group will be incentivized to collude to steal the money in peg wallet even forfeiting the reward incentive.
