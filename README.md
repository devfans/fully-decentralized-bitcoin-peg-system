...
* Operator should keep an eye on the status of collateral to ensure the value is always enough so that the ratio wont go above LF(LF \~= 85%? LF is the ratio where a liquidation will be initialized).  Liquidation could be triggered by price change or malicious behavior of the operator which attempts to operate the received BTC(UTXOs) in peg wallet. Liquidation could be implemented in the bridge contract, like open a order which allows any use to burn same amount of peg-BTC to exchange for the full operator collateral. Liquidation also helps to maintain the pegged token to be 1:1 as the BTC. Liquidation request could be filled by arbitrators with a potential profit of 1-LF(\~= 15% of the volume). Liquidation could also be implemented with dexs on layer 2 chains.
